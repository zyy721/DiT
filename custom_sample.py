...
def main(args):
    ...
    for batch_dict in val_dataset_loader:
        ...
        z = torch.randn(*latents.shape, device=device)
        cond_frame = latents
        ...
        samples = diffusion.p_sample_loop(
            model.custom_forward_with_cfg, z.shape, z, clip_denoised=False, model_kwargs=model_kwargs, progress=True, device=device
        )

        frames = samples.view(batch_size, nframes, *samples.shape[1:])
        frames_pred = frames[:, nframes_past:]
